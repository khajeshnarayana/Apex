...
class JobDescriptionAdapterTests(unittest.TestCase):
    def test_current_backend_parser_is_the_only_jd_classification_path(self) -> None:
        ...
        result = build_job_description("Backend role", parse)

        self.assertIs(result, backend_result)
        self.assertEqual(observed, ["Backend role"])

    def test_current_backend_can_classify_unlabelled_jd_context(self) -> None:
        result = build_job_description("Build Python REST APIs with PostgreSQL.")

        self.assertTrue(result["required_skills"])
        self.assertIn("Python", result["required_skills"])
        self.assertIn("REST API", result["required_skills"])

    def test_empty_or_unclassified_jd_fails_cleanly(self) -> None:
        with self.assertRaisesRegex(UploadPipelineError, "no extractable text"):
            build_job_description("   ")
        with self.assertRaisesRegex(UploadPipelineError, "No required skills"):
            build_job_description("Operations role with stakeholder communication.")
...
class UploadPipelineIntegrationTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.jd_parser = staticmethod(parse_jd)
